package br.com.estatistica.reflec;
...
public class ClassIntrospector {
	...
	public <E extends Annotation> List<AnnotatedElement<Method, E>> getAnnotatedDeclaredMethods(Class<E> ann) {
		...
		for (Method f : this.clazz.getDeclaredMethods()) {
			if (f.isAnnotationPresent(ann)) {
	...
	public <E extends Annotation> List<AnnotatedElement<Method, E>> getAnnotatedMethods(Class<E> ann) {
		List<AnnotatedElement<Method, E>> list = new ArrayList<AnnotatedElement<Method, E>>();
		for (Method f : this.clazz.getMethods()) {
			if (f.isAnnotationPresent(ann)) {
				list.add(new AnnotatedElement<Method, E>(f, f.getAnnotation(ann)));
			}
		}
	...
	public Class<?> getMethodReturnClass(String string, Class<?> arg) {
		try {
			Method m = this.clazz.getDeclaredMethod(string, arg);
	...
	private Field getField(String fieldName, Class<?> clazz) throws NoSuchFieldException {
		...
		try {
			...
			return this.getField(fieldName, clazz.getSuperclass());
		}
